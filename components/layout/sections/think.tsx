    ...
    <section id="benefits" className="container md:py-16 w-full">
      <div className="mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-2 mt-12">
              ...
              <CarouselContent className="w-full">
                {benefitList.map(({ icon, title, description }, index) => (
                  <CarouselItem key={index} className="w-full sm:max-w-[60%] mx-auto">
                    <Card className="bg-white hover:shadow-md transition-all duration-300 rounded-2xl h-full">
          ...
          <div className="hidden lg:grid grid-cols-1 sm:grid-cols-2 gap-2">
            {benefitList.map(({ icon, title, description }) => (
              <Card key={title} className="bg-white hover:shadow-md transition-all duration-300 rounded-2xl h-full">
                <CardHeader className="pb-2">
                  <div className="flex justify-between items-start gap-2">
                    <CardTitle className="text-lg text-[#131426] font-semibold">{title}</CardTitle>
                    <Icon
